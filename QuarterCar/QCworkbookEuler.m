...
%%
[w_l,linear_analytic]=SS.compute_linear_PSD(PSDpair,[0,25],false);
...
%%
omega.w = w; omega.linear = w_l; omega.wss = wss;
Gxx.Gfull = outputPSD; Gxx.linear_analytic = linear_analytic; Gxx.Gss = ssmPSD; 
plot_log_PSD(omega,Gxx,order,PSDpair,[0 30],false)
%%
[U, ~, NOT_CONVERGED] = eigs(sparse(K),sparse(M),2,'smallestabs');
...
parfor i=1:nRealization
    ...
    [w_g,Gz_g] = GalerkinProj (V, SS, M, C, K);
...
pool.delete()
%%
char=['nonlQCEpsilon',num2str(epsilon),'T',num2str(T0),'nP',num2str(log(nPoints)/log(2)),'.mat'];
save(char)
...
% set(SS,'M',M,'C',C,'K',K,'fnl',fnl);
% set(SS.Options,'Emax',5,'Nmax',10,'notation','multiindex')
% set(SS.SSOptions,'ssMethod','direct')
% SS.add_random_forcing(nRealization, T0, nPoints,forcingdof);
% 
% %%%%%%%% Above is forcing setting and set to DynamicalSystem class
% 
...
% omega.w = w; omega.linear = w_l; omega.wss = wss;
% Gxx.Gfull = outputPSD; Gxx.linear_analytic = linear_analytic; Gxx.Gss = ssmPSD; 
% plot_log_PSD(omega,Gxx,order,PSDpair,[0 25],false);
% charf=['ep',num2str(epsilon),'.png'];
% saveas(gcf,charf);
% %%
% char=['QC',num2str(epsilon),'T',num2str(T0),'nP',num2str(log(nPoints)/log(2)),'.mat'];
% save(char)
% 
% epsilon=epsilon+100;
% end